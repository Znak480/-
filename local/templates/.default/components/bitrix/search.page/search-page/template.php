		...
		<div class="search-advanced">
			<div class="search-advanced-result">
				<?if(is_object($arResult["NAV_RESULT"])):?>
					...
					<?echo GetMessage("CT_BSP_FOUND")?>: <span class="js-count-result"><?echo $arResult["NAV_RESULT"]->SelectedRowsCount()?></span>
					</div>
				<?endif;?>
				<?
				$arWhere = array();
	...
	<?elseif(count($arResult["SEARCH"])>0):?>
		...
		<?foreach($arResult["SEARCH"] as $arItem):?>
				<div class="product">
					...
					<?$file = CFile::ResizeImageGet($ar_res["PREVIEW_PICTURE"], array('width'=>170, 'height'=>170), BX_RESIZE_IMAGE_PROPORTIONAL, true); ?>
					<div class="image">
						<a href="<?echo $arItem["URL"]?>"><img src="<?=$file['src']?>" alt=""></a>
					</div>
					<div class="name">
						<a href="<?echo $arItem["URL"]?>"><?echo $arItem["TITLE_FORMATED"]?></a> 
					</div>
					<div class="buttons">
						<input type="number" class="quantity"  min="1" id="quant<?=$arItem["ITEM_ID"]?>" value="1">
						<input type="button" value="В корзину" class="add-to-cart" rel="<?=$arItem["ITEM_ID"]?>">
					</div>
					<div class="comparison">
						<input type="checkbox"> <label for="">Добавить к сравнению</label>
					</div>
				</div>
