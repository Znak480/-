...
if (0 < $arResult['SECTIONS_COUNT'])
{
	...
	if ($boolPicture || $boolDescr)
	{
		...
		//ov_dump($_GET['q']);
		//ov_dump($arParams['OV_SEARCH_TEXT']);

		$search_array = explode(' ', $arParams['OV_SEARCH_TEXT'] );
		$sucess = [];

		foreach ($arResult['SECTIONS'] as $key => $arSection)
		{
			if(isset($arParams['OV_SEARCH_TEXT'])){
				...
				foreach($search_array as $item_search){
					if(strlen($item_search) > 4){
						...
						//echo '<p>'.$arSection['NAME'].' '.$sub_search.'</p>';
						//var_dump(stripos( mb_strtoupper($arSection['NAME']), mb_strtoupper($sub_search) ));
						if( stripos( mb_strtoupper($arSection['NAME']), mb_strtoupper($sub_search) ) === false ){
							if(!in_array($key, $sucess)){
