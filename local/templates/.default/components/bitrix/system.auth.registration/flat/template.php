...
<div class="bx-authform">
	<h3 class="bx-title mb-30">Регистрация</h3>
<noindex>

<?
//echo '<pre>'; var_dump($arParams["AUTH_RESULT"]["MESSAGE"]); echo '</pre>';
//$test = array_unique(explode("&lt;br&gt;", $arParams["AUTH_RESULT"]["MESSAGE"]));
//implode($test);
//$text = echo '<pre>'; print_r($test); echo '</pre>';
...
<?elseif(!$arResult["SHOW_EMAIL_SENT_CONFIRMATION"]):?>
	...
	<form data-reg-form method="post" action="<?=$arResult["AUTH_URL"]?>" name="bform" enctype="multipart/form-data" onSubmit="$('#fieldlogin').val($('#fieldemail').val()
		...
		<div class="bx-authform-formgroup-container" style="display: none;">
			...
			<div class="bx-authform-input-container">
				<input id="fieldlogin" type="text" name="USER_LOGIN" maxlength="255" value="<?=$arResult["USER_LOGIN"]?>" />
			</div>
		</div>
...
<?if($arResult["EMAIL_REGISTRATION"]):?>
		<div class="bx-authform-formgroup-container">
			...
			<div class="bx-authform-input-container">
				<input id="fieldemail" type="text" name="USER_EMAIL" maxlength="255" value="<?=$arResult["USER_EMAIL"]?>" />
			</div>
		</div>
<?endif?>
