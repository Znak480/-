...
<head>
	...
	<?php
	$mobiles = ["iPhone", "iPad", "iPod", "Android", "webOS", "BlackBerry", "IEMobile", "PlayBook", "BB10", "Nokia"];
	global $desktop;
	$desktop = true;
	...
	Asset::getInstance()->addJs(SITE_TEMPLATE_PATH.'/js/develop.js');
	?>
...
<div class="header regular-block header_e1">
	...
	<div class="header_e1__wrapper">
		<div class="logo-box">
			<a href="/">
				<img src="/i/logo.gif" alt="" class="logo">
			</a>
			<div class="title">Для тех, кто строит и ремонтирует</div>
		</div>
		...
		<div class="flex-shrink-0">
			<a href="/cart/" class="cart cart_e1" id="cart">
				<? $APPLICATION->IncludeComponent("bitrix:sale.basket.basket.small", "small", [
					"COLUMNS_LIST"       => ["PRICE_FORMATED"],
					"COMPONENT_TEMPLATE" => ".default",
					"PATH_TO_BASKET"     => "/cart/",
					"PATH_TO_ORDER"      => "",
					"SHOW_DELAY"         => "Y",
	...
	<? if($APPLICATION->GetCurPage(false) === '/' || $data[1] == "catalog"): ?>
		<div class="lb">
			...
			if(strstr($APPLICATION->GetCurDir(), '/catalog/') and $APPLICATION->GetCurPage(false) !== '/catalog/' and (count($currUrl) == 4 or count($currUrl) == 5))
			{
				...
				$arSelect = ["ID", "NAME"];
				$arFilter = ["IBLOCK_ID" => $currentCity['IBLOCK_CATALOG']['VALUE'], "ACTIVE_DATE" => "Y", "ACTIVE" => "Y", "CODE" => end($currUrl)];
			...
			<div class="catalog <?=$catalogHide?>">
				...
				<ul class="parent topCatalog">
					<? $arFilter = ['IBLOCK_ID' => $currentCity['IBLOCK_CATALOG']['VALUE'], "ACTIVE" => "Y", "SECTION_ID" => false];
					...
					while($arSect = $rsSect->GetNext())
					{
						...
						<li class="parent">
							...
							<? $arFilter2 = ['IBLOCK_ID' => $currentCity['IBLOCK_CATALOG']['VALUE'], "ACTIVE" => "Y", "SECTION_ID" => $arSect["ID"]];
							$rsSect2 = CIBlockSection::GetList(["SORT" => "ASC"], $arFilter2);
							$i = 0;
										...
										<div class="sub-catalog__block">
											...
											<ul class="child">
												<li class="child-title">
													<a href="<?=$arSect2["SECTION_PAGE_URL"]?>"><?=$arSect2["NAME"]?></a>
												</li>
												<? $arFilter3 = ['IBLOCK_ID' => $currentCity['IBLOCK_CATALOG']['VALUE'], "ACTIVE" => "Y", "SECTION_ID" => $arSect2["ID"]];
												$rsSect3 = CIBlockSection::GetList(["SORT" => "ASC"], $arFilter3);
												while($arSect3 = $rsSect3->GetNext())
												{
													?>
													<li class="child">
														<a href="<?=$arSect3["SECTION_PAGE_URL"]?>"><?=$arSect3["NAME"]?></a>
													</li>
												<? } ?>
											</ul>
											<?
