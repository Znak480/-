...
$parentIds = array_column($arResult['SECTIONS'], 'ID');
...
if (!empty($parentIds)) {
    $rsChildren = CIBlockSection::GetList(
        ...
        false,
        ["ID", "NAME", "CODE", "SECTION_PAGE_URL", "IBLOCK_SECTION_ID"]
    );
    
    $childrenByParent = [];
    while ($child = $rsChildren->GetNext()) {
        $parentId = $child["IBLOCK_SECTION_ID"];
    ...
    foreach ($arResult['SECTIONS'] as $key => $arSection) {
        ...
        $arResult['SECTIONS'][$key]["CHILDREN"] = 
            $childrenByParent[$sectionId] ?? []; 
    }
}
