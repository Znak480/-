...
$arResizeParams = array(
    "width"  => intval($arParams["IMAGE_WIDTH"] ?: 800),
    "height" => intval($arParams["IMAGE_HEIGHT"] ?: 600),
);
...
foreach ($pictures as $pictureId) {
	...
	$picPath = CFile::GetPath($pictureId);

    if ($picPath && $arImage && !empty($arImage["src"])) {
		$arImage["BASE_PATH"] = $picPath;

		if (!isset($arResult["IMAGES"])) {
            $arResult["IMAGES"] = [];
        }
	...
	if ($arThumb) {
        $arResult["THUMBS"][] = $arThumb;
    }
}

if (!empty($arResult["IMAGES"])) {
    $arResult["FIRST_IMAGE"] = $arResult["IMAGES"][0];
}
unset($pictures);

$tipsIds = $arResult["PROPERTIES"]["TIPS"]["VALUE"] ?? null;

if (!is_array($tipsIds)) {
    $tipsIds = !empty($tipsIds) ? [$tipsIds] : [];
}
$tipsIds = array_filter($tipsIds);

$hasTips = !empty($tipsIds);
$arTips = [];

if($hasTips){
	$cacheId = "tips_iblock7_" . implode('_', $tipsIds);
    $cachePath = "/tips_cache/";
    $cacheTime = 3600; // 1 час
    
    $cache = new CPHPCache();

	if($cache->InitCache($cacheTime, $cacheId, $cachePath)){
		$arTips = $cache->GetVars();
	}else{
		$rsTips = CIBlockElement::GetList(
			["SORT" => "ASC"],
			[
				"IBLOCK_ID" => "7",
				"ACTIVE" => "Y",
		...
		while ($arElement = $rsTips->GetNext()) {
			$arElement["PICTURE_PATH"] = $arElement["PREVIEW_PICTURE"] 
				? CFile::GetPath($arElement["PREVIEW_PICTURE"]) 
				:null; 

			$arElement["RESIZED_PICTURE_PATH"] = $arElement["PREVIEW_PICTURE"] 
        ...
        $cache->EndDataCache($arTips);
	}
}

$arResult["TIPS"] = [
	"HAS_TIPS" => $hasTips && !empty($arTips),
    "IDS" => $tipsIds, 
    "ITEMS" => $arTips,  
    "COUNT" => count($arTips),
    "TOTAL_COUNT" => count($tipsIds),
	"MISSING_COUNT" => count($tipsIds) - count($arTips),
];


if (empty($arResult['DISPLAY_PROPERTIES']) && !empty($arResult['PROPERTIES'])) {
    $excludedCodes = ['PICTURES', 'GALLERY', 'TIPS', 'NA_SAYT', 'KOD_TOVARA', 'POISKOVYY_KONTENT', 'ARTIKUL', 'SEZON_ISPOLZOVANIYA',]; 
    ...
    foreach ($arResult['PROPERTIES'] as $key => $arProp) {
        ...
        if (in_array($key, $excludedCodes)) continue;
        
       
        $arResult['DISPLAY_PROPERTIES'][$key] = $arProp;
        
      
        if ($arProp['PROPERTY_TYPE'] == 'L') {
            $arResult['DISPLAY_PROPERTIES'][$key]['DISPLAY_VALUE'] = $arProp['VALUE_ENUM'];
        }
    }
}
