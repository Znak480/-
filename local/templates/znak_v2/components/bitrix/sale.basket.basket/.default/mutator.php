...
PriceProcessor::processPrices($this->basketItems);

$totalCustomSum = 0;
$totalCustomDiscountSum = 0;
$totalCustomSaving = 0;
$totalCustomDiscountPercent = 0;
$itemsWithDiscount = 0;
...
foreach ($this->basketItems as $row)
{
	...
	$priceData = $row["PRICE_DATA"] ?? null;

    if (!empty($priceData)) {
        $currentPrice = $priceData['CURRENT_PRICE'] ?? 0;
        $discountPrice = $priceData['DISCOUNT_PRICE'] ?? 0;
        $discountPercent = $priceData['DISCOUNT_VALUE'] ?? 0;
        $discountType = $priceData['DISCOUNT_TYPE'] ?? 'none';
        
        $rowData['CUSTOM_CURRENT_SUM'] = $rowData['QUANTITY'] * $currentPrice;
        $rowData['CUSTOM_DISCOUNT_SUM'] = $rowData['QUANTITY'] * $discountPrice;
        $rowData['CUSTOM_DISCOUNT_PERCENT'] = $discountPercent;
        $rowData['CUSTOM_DISCOUNT_TYPE'] = $discountType;
        
        $rowData['CUSTOM_CURRENT_SUM_FORMATED'] = CCurrencyLang::CurrencyFormat(
            $rowData['CUSTOM_CURRENT_SUM'], 
            $rowData['CURRENCY'], 
            true
        );
        $rowData['CUSTOM_DISCOUNT_SUM_FORMATED'] = CCurrencyLang::CurrencyFormat(
            $rowData['CUSTOM_DISCOUNT_SUM'], 
            $rowData['CURRENCY'], 
            true
        );
        
        $rowData['CUSTOM_HAS_DISCOUNT'] = $discountPrice < $currentPrice && $discountPrice > 0;
        
        $rowData['CUSTOM_SAVING'] = ($currentPrice - $discountPrice) * $rowData['QUANTITY'];
        $rowData['CUSTOM_SAVING_FORMATED'] = CCurrencyLang::CurrencyFormat(
            $rowData['CUSTOM_SAVING'], 
            $rowData['CURRENCY'], 
            true
        );

		$totalCustomSum += $rowData['CUSTOM_CURRENT_SUM'];
        $totalCustomDiscountSum += $rowData['CUSTOM_DISCOUNT_SUM'];
        $totalCustomSaving += $rowData['CUSTOM_SAVING'];
        
        if ($rowData['CUSTOM_HAS_DISCOUNT']) {
            $itemsWithDiscount++;
        }
	
    } else {
        $rowData['CUSTOM_CURRENT_SUM'] = $rowData['SUM_PRICE'];
        $rowData['CUSTOM_DISCOUNT_SUM'] = $rowData['SUM_PRICE'];
        $rowData['CUSTOM_DISCOUNT_PERCENT'] = 0;
        $rowData['CUSTOM_DISCOUNT_TYPE'] = 'none';
        $rowData['CUSTOM_CURRENT_SUM_FORMATED'] = $rowData['SUM_PRICE_FORMATED'];
        $rowData['CUSTOM_DISCOUNT_SUM_FORMATED'] = $rowData['SUM_PRICE_FORMATED'];
        $rowData['CUSTOM_HAS_DISCOUNT'] = false;
        $rowData['CUSTOM_SAVING'] = 0;
        $rowData['CUSTOM_SAVING_FORMATED'] = '';

		$totalCustomSum += $rowData['SUM_PRICE'];
        $totalCustomDiscountSum += $rowData['SUM_PRICE'];
    }
...
$totalData = array(
	...
	'CUSTOM_SUM' => $totalCustomSum,
    'CUSTOM_SUM_FORMATED' => CCurrencyLang::CurrencyFormat($totalCustomSum, $result['CURRENCY'], true),
    'CUSTOM_DISCOUNT_SUM' => $totalCustomDiscountSum,
    'CUSTOM_DISCOUNT_SUM_FORMATED' => CCurrencyLang::CurrencyFormat($totalCustomDiscountSum, $result['CURRENCY'], true),
    'CUSTOM_SAVING' => $totalCustomSaving,
    'CUSTOM_SAVING_FORMATED' => CCurrencyLang::CurrencyFormat($totalCustomSaving, $result['CURRENCY'], true),
    'CUSTOM_ITEMS_WITH_DISCOUNT' => $itemsWithDiscount,
    'CUSTOM_TOTAL_ITEMS' => count($this->basketItems),
);
