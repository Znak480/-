...
<?//наши преимущества (иконки под баннером)?>
	<div class="feautures d-none">
	...
	<div class="popular regular-block category-page">
            ...
            function compare_tov(id)
            {
                ...
                if (chek)
                {
                    ...
                    $.get("/ajx/list_compare.php",
                        ...
                        function(data) {
                            thisElem.html(data);
                            console.log('dsvsdvsdvsdvsdvdfkjnb ljsfnb ');
                            console.log(data);
        ...
        <h2 class="din">Популярные товары</h2>
		<div class="popular__list">
            ...
            $arFilter = Array("!=PROPERTY_RAITING_PRODAZH" => false, "IBLOCK_ID"=>$currentCity['IBLOCK_CATALOG']['VALUE'], "ACTIVE_DATE"=>"Y", "ACTIVE"=>"Y",'!NAME'=>false/*,"!=PREVIEW_PICTURE"=>false*/);//"!=PROPERTY_RAITING_PRODAZH"=>false,

/*            $arFilter[]=array(
                "LOGIC" => "OR",
                array("!=PROPERTY_RAITING_PRODAZH" => false),
                array("!=AKTSIYA_" => false)
            );
*/
  			$arrSectionPopular = [];
            $count_popular = 0;
            $res = CIBlockElement::GetList(Array("RAND"=>"RAND"), $arFilter, false, Array("nTopCount"=>400), $arSelect);
            while($ob = $res->GetNextElement())
            {
            	if($count_popular == 4) break;
                //echo 125125;
                $arFields = $ob->GetFields();
                if(!in_array($arFields['IBLOCK_SECTION_ID'],$arrSectionPopular)){
                    $arrSectionPopular[] = $arFields['IBLOCK_SECTION_ID'];
	...
	//                if(empty($arFields['PROPERTY_SALE_VALUE']) or empty($arFields['PROPERTY_DISCOUNT_VALUE']) or empty($arFields['PROPERTY_HIT_VALUE']))
	//                    continue;
                    //debug($arFields);
	            ?>
	                <?
	...
	<div class="news regular-block">
		<h2 class="din"><a class="color-black" href="/stocks/">Акции</a></h2>
		<div class="news__wrap">
        <?$APPLICATION->IncludeComponent("bitrix:news.list", 	"news_stock_index", 
 Array(
            "ACTIVE_DATE_FORMAT" => "d.m.Y",	// Формат показа даты
            "ADD_SECTIONS_CHAIN" => "Y",	// Включать раздел в цепочку навигации
            "AJAX_MODE" => "N",	// Включить режим AJAX
            ...
            "FILTER_NAME" => "",	// Фильтр
            "HIDE_LINK_WHEN_NO_DETAIL" => "N",	// Скрывать ссылку, если нет детального описания
            "IBLOCK_ID" => $currentCity["IBLOCK_ACTIONS"]["VALUE"],	// Код информационного блока
            "MESSAGE_404" => "",	// Сообщение для показа (по умолчанию из компонента)
            "NEWS_COUNT" => "4",	// Количество новостей на странице
            "PAGER_BASE_LINK_ENABLE" => "N",	// Включить обработку ссылок
            "PAGER_DESC_NUMBERING" => "N",	// Использовать обратную навигацию
...
<?/*
	<!-- <div class="shares">
		<h2 class="din" style="cursor:pointer;" onclick="location.href='/stocks/'">Наши акции</h2>
		<?$APPLICATION->IncludeComponent(
	"bitrix:news.list",
	"stocks_mainpage",
	array(
		"COMPONENT_TEMPLATE" => "stocks_mainpage",
		"IBLOCK_TYPE" => "news",
		"IBLOCK_ID" => "5",
		"NEWS_COUNT" => "3",
		"SORT_BY1" => "ACTIVE_FROM",
...
*/?>
 ...
 <?$APPLICATION->IncludeComponent("bitrix:news.list", "service-list", Array(
	"COMPONENT_TEMPLATE" => ".default",
		"IBLOCK_TYPE" => "news",	// Тип информационного блока (используется только для проверки)
		"IBLOCK_ID" => $currentCity['IBLOCK_SERVICES']['VALUE'],	// Код информационного блока
...
<?/*
	...
	<!-- <div class="advice regular-block">
		<h2 class="din" style="cursor:pointer;" onclick="location.href='/recommends/'">Советы от компании «Знак»</h2>
		<?$APPLICATION->IncludeComponent(
	"bitrix:news.list",
	"recommends_mainpage",
	array(
		"COMPONENT_TEMPLATE" => "recommends_mainpage",
		"IBLOCK_TYPE" => "news",
		"IBLOCK_ID" => "7",
		"NEWS_COUNT" => "10",
		"SORT_BY1" => "ACTIVE_FROM",
